...
class AttendanceController extends Controller
{
    ...
    public function update(Request $request, $attendance_id) {
        ...
        //$hour=DB::select("SELECT TIMESTAMPDIFF(HOUR,created_at,updated_at) AS diff_hour FROM attendances where id ='$attendance_id'");
        $hour=DB::select("SELECT TIMESTAMPDIFF(SECOND,created_at,updated_at) AS diff_hour FROM attendances where id ='$attendance_id'");
        $w_hour = json_decode(json_encode($hour), true);
        $dw_hour=$w_hour[0]['diff_hour'];
        $attendance->work_hour=$dw_hour;
        $attendance= Attendance::where('id','=',$attendance_id)->update(['work_hour'=>$dw_hour]);
        //$attendance->save();
